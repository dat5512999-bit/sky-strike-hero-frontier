(function (ns) {
  ...
  $('help').addEventListener('click', () => {
    const block = el('div'); block.append(el('p', 'eyebrow', '圖鑑使用說明'), el('h2', '', '世界仍有未解之謎'));
    const list = el('ul', 'help-list');
    ['完全未知：只有未明記錄；曾遇見後會留下模糊身影。', '已發現：可以閱讀基本資料，完整數值與能力仍可能未知。', '已解鎖 / 完整資料：可使用，或已取得目前完整資料；不代表揭曉所有世界真相。', '戰術定位提供能力搭配建議；詳細數值可展開查看真實數值。', '點選卡片閱讀；手機可按「返回」或 Esc 回到收藏。', '編年史只顯示目前取得的史料，後續線索會追加而不取代舊記錄。', '「新資訊」是新收錄，「史料更新」代表讀過的條目有新史料。', '史料標籤代表來源；看完可按「標記本頁史料已讀」。', '收藏儲存在目前瀏覽器；更換瀏覽器或清除資料不會自動移轉。'].forEach(text => list.append(el('li', '', text)));
    block.append(list); openModal(block);
  });
  const archiveSymbols = { WORLD: '◎', CIVILIZATION: '⚑', EVENT: '◇', HISTORY: '◷', BELIEF: '✧', LEGEND: '✦', CHARACTER_RECORD: '♜' };
  const knowledgeLabels = { UNKNOWN: '完全未知', ENCOUNTERED: '已遇見', DISCOVERED: '已發現', UNLOCKED: '已解鎖', FULLY_KNOWN: '完整資料' };
  const archiveLabel = value => value === 'ARCHIVE UPDATED' ? '史料更新' : value === 'NEW INFORMATION' ? '新資訊' : value;
  const playerText = value => String(value ?? '').replaceAll('STORY_LOCKED', '劇情尚未開放').replaceAll('CONCEPT', '概念設定').replaceAll('TESTABLE', '可測試').replace(/\bFrost\b/g, '寒冷').replace(/\bBoss\b/g, '首領');
  const eras = [['ancient', '遠古歷史'], ['pre-chapter', '故事之前'], ['chapter1', '第一章'], ['chapter2', '第二章'], ['future', '後續篇章']];
  function renderCategories() {
    $('categories').replaceChildren(...api.categories.map(([id, name, english, icon]) => {
      ...
      if (unread.some(key => key.startsWith(id + ':') || id === 'chronicle' && key.startsWith('cinematic:'))) node.append(el('span', 'nav-new', '新'));
      return node;
    }));
  }
  ...
  function makeCard(entry) {
    ...
    if (!entry.archive && progress.isUnread(entry.id)) node.append(el('span', 'archive-badge', '新內容'));
    const marker = entry.archive ? entry.readable ? entry.chronicleCategory : '封存記錄' : knowledgeLabels[entry.knowledgeLevel];
    node.append(el('span', 'card-marker', marker));
    if (entry.updateLabel) node.append(el('span', 'archive-badge', archiveLabel(entry.updateLabel)));
  ...
  function renderCollection() {
    ...
    $('collection-title').textContent = config[1] + '收藏'; $('category-en').textContent = config[2] + '／世界圖鑑';
    ...
    $('archive-modes').replaceChildren(...[['collection', '收藏'], ['timeline', '時序']].map(([mode, label]) => {
    ...
    if (category === 'chronicle' && progression?.save.data.unlockedCinematics.length) {
      $('archive-modes').append(button('劇情典藏' + (progression.unread().some(id => id.startsWith('cinematic:')) ? ' · 新內容' : ''), () => {
        const content = el('div'); content.append(el('h2', '', '劇情典藏'));
        progression.save.data.unlockedCinematics.forEach((id, index) => {
          ...
          if(entry){content.append(el('h3','',entry.name),el('p','','已確認／故事體驗'),button('重播影片',async()=>{closeModal();const result=await integration.replay(entry);notify(result.message);},'primary'));}
  ...
  function facts(rows) { const dl = el('dl', 'facts'); rows.forEach(({ label, value }) => { const row = el('div'); row.append(el('dt', '', label), el('dd', '', playerText(value))); dl.append(row); }); return dl; }
  ...
  function showRelated(node, entry) {
    ...
    const relatedLabels = { hero: '相關人物', faction: '相關軍團', map: '相關地圖', chronicle: '延伸編年史' };
  ...
  function basic(node, entry) {
    ...
    if (entry.identity && !['unit', 'tower'].includes(entry.type)) node.append(el('p', 'identity', playerText(entry.identity)));
    ...
    node.append(chips);
    node.append(el('p', 'knowledge-label', knowledgeLabels[entry.knowledgeLevel] || entry.knowledgeLevel));
    ...
    if (['hero', 'unit', 'tower', 'enemy'].includes(entry.type)) {
      if (entry.full) {
        const stats = el('details', 'detailed-stats'); stats.append(el('summary', '', '詳細數值'));
        const source = entry.source;
        const rows = [['damage', '攻擊'], ['health', '生命'], ['range', '射程'], ['interval', '攻擊間隔'], ['cost', '金幣'], ['wood', '木材'], ['armor', '護甲'], ['speed', '移動速度'], ['splash', '影響範圍'], ['attackDamage', '對英雄攻擊'], ['attackInterval', '對英雄攻擊間隔'], ['reward', '擊殺金幣']].filter(([key]) => source[key] !== undefined).map(([key, label]) => ({ label, value: String(source[key]) + (key === 'interval' || key === 'attackInterval' ? ' 秒' : '') }));
  ...
  function loreView(node, entry) {
    ...
    records.forEach((record, index) => {
      ...
      if (record.isNew) top.append(el('span', 'new-fragment', '新內容'));
  ...
  function archiveView(node, entry) {
    if (entry.updateLabel) node.append(el('p', 'archive-update', archiveLabel(entry.updateLabel)));
    node.append(el('p', 'eyebrow', entry.chronicleCategory + '／' + (entry.readable ? '已收錄 ' + entry.lore.length + ' 段史料' : '封存記錄')));
    if (entry.readable) {
      node.append(facts([{ label: '故事章節', value: entry.chapter ? '第 ' + entry.chapter + ' 章' : '世界公開記錄' }, { label: '史料來源', value: [...new Set(entry.lore.map(row => api.sources[row.sourceType]))].join('／') }, { label: '收錄狀態', value: entry.archiveStatus }]));
      heading(node, '目前所知'); node.append(el('p', 'archive-summary', entry.lore[0] ? '【' + api.sources[entry.lore[0].sourceType] + '】' + entry.lore[0].text.split('。')[0] + '。' : api.emptyLore));
      heading(node, '✓ 已確認史實');
  ...
  function renderDetail() {
    ...
    if (entry.updateLabel) title.append(el('span', 'archive-banner-update', archiveLabel(entry.updateLabel)));
