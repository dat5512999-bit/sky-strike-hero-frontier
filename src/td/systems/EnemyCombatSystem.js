(function(ns){
  'use strict';
  const BOSS_REINFORCEMENTS={
    nagaSiltwaterDemonlord:['nagaSiltImp','frostRimeStalker','goblinSapper','orcBanner'],
    'boss:tide':['nagaSiltImp','nagaShellbreaker','healer','commander'],
    'boss:siege':['goblinSapper','goblinVeilrunner','orcBanner','warder'],
    'boss:frost':['frostRimeStalker','frostRimePriest','frostSkyraider','warder']
  };
  class EnemyCombatSystem{
    ...
    phase(monster,monsters,hooks){
      if(monster.combatRole!=='boss'||monster.evolutionTrial||monster.evolutionTraining)return;
      ...
      if(next===2&&!monster.phaseTriggered[2]){
        monster.phaseTriggered[2]=true;const reinforcements=[],types=BOSS_REINFORCEMENTS[monster.type]||BOSS_REINFORCEMENTS['boss:'+monster.bossAspect]||(monster.wave>=30?['runner','commander','healer','runner']:monster.wave>=25?['runner','brute','commander']:monster.wave>=20?['runner','commander','runner']:['runner','runner']);
        ...
        if(monster.bossAspect==='tide')monsters.filter(unit=>unit.active&&unit!==monster&&ns.utils.distance(unit,monster)<=150).forEach(unit=>{unit.barrier=Math.max(unit.barrier||0,Math.round(unit.maxHealth*.18));unit.maxBarrier=Math.max(unit.maxBarrier||0,unit.barrier);});
        if(monster.bossAspect==='frost')monsters.filter(unit=>unit.active&&unit!==monster&&ns.utils.distance(unit,monster)<=155).forEach(unit=>{unit.bossWardTime=Math.max(unit.bossWardTime||0,7);});
    ...
    bossAbility(dt,monster,hero,defenders,hooks){
      ...
      monster.abilityCooldown-=dt;if(monster.abilityCooldown<=0){monster.abilityWindup=1.1;monster.combatTarget=null;monster.attackWindup=0;monster.attackVisualTime=0;monster.attackVisualDuration=0;const labels={tide:'濁潮護幕',siege:'震爆迫擊',frost:'霜骨封鎖',demonlord:'深潮踐踏'};if(hooks&&hooks.onWarning)hooks.onWarning(monster,labels[monster.bossAspect]||'戰爭踐踏',monster.abilityRadius);return true;}return false;
    }
