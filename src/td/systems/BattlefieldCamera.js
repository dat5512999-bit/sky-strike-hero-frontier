(function(ns){
  ...
  class BattlefieldCamera {
    constructor(width,height){
      ...
      this.inspect=false;this.follow=false;this.space=false;this.gesture=null;this.touches=new Map();this.pinchDistance=0;
    }
    ...
    initialView(options){const focus=this.map&&this.map.openingFocus||{x:this.worldWidth/2,y:this.worldHeight/2},preferFocus=Boolean(options&&options.preferFocus),initialZoom=this.map&&this.map.camera&&this.map.camera.mobileInitialZoom||1;if(this.canAutoFit()&&!preferFocus)this.showAll();else{this.overview=false;this.zoom=preferFocus?initialZoom:1;this.follow=false;this.inspect=false;this.focus(focus.x,focus.y);}this.started=true;return this.overview;}
    ...
    reset(){this.zoom=1;this.overview=false;this.follow=false;this.inspect=false;this.space=false;this.gesture=null;this.touches.clear();this.pinchDistance=0;this.started=false;this.focus(this.worldWidth/2,this.worldHeight/2);}
    ...
    begin(game){
      ...
      if(!this.started){const requestedFollow=this.follow,mobileFocus=body.dataset.layout==='mobile'&&body.dataset.combatOrientation==='landscape';this.initialView({preferFocus:mobileFocus});if(requestedFollow)this.follow=true;}
    ...
    attach(game,ui){
      ...
      const finishTouch=event=>{
        const id=event&&event.pointerId;if(id===undefined||!camera.touches.has(id))return false;
        camera.touches.delete(id);if(canvas.hasPointerCapture(id))canvas.releasePointerCapture(id);
        const remaining=Array.from(camera.touches.entries());
        camera.gesture=remaining.length===1?{id:remaining[0][0],x:remaining[0][1].x,y:remaining[0][1].y,touch:true}:null;
        camera.pinchDistance=0;return true;
      };
      ...
      canvas.addEventListener('pointerdown',event=>{
        ...
        if(event.pointerType==='touch'){
          ...
          if(points.length===1)camera.gesture={id:event.pointerId,x:event.clientX,y:event.clientY,touch:true};
          else{camera.gesture=null;camera.pinchDistance=Math.hypot(points[0].x-points[1].x,points[0].y-points[1].y);event.stopImmediatePropagation();}
          camera.clamp();sync();return;
        }
      ...
      canvas.addEventListener('pointermove',event=>{
        if(event.pointerType==='touch'&&camera.touches.has(event.pointerId)){
          event.preventDefault();event.stopImmediatePropagation();
          if(blocked()){finishTouch(event);return;}
          const previous=camera.touches.get(event.pointerId);camera.touches.set(event.pointerId,{x:event.clientX,y:event.clientY});const points=Array.from(camera.touches.values());
          if(points.length>=2){const mx=(points[0].x+points[1].x)/2,my=(points[0].y+points[1].y)/2,distance=Math.hypot(points[0].x-points[1].x,points[0].y-points[1].y),rect=canvas.getBoundingClientRect(),sx=(mx-rect.left)*camera.width/rect.width,sy=(my-rect.top)*camera.height/rect.height,before=camera.screenToWorld(sx,sy);if(camera.pinchDistance>0)camera.changeZoom(Math.max(.82,Math.min(1.22,distance/camera.pinchDistance)));const after=camera.screenToWorld(sx,sy);camera.x+=before.x-after.x;camera.y+=before.y-after.y;camera.clamp();camera.pinchDistance=distance;}
          else camera.pan(event.clientX-previous.x,event.clientY-previous.y);
          sync();return;
        }
        if(!camera.gesture)return;
        event.preventDefault();event.stopImmediatePropagation();
      ...
      ['pointerup','pointercancel','lostpointercapture'].forEach(type=>canvas.addEventListener(type,event=>{
        if(finishTouch(event)){event.stopImmediatePropagation();return;}if(!camera.gesture)return;event.stopImmediatePropagation();finish(event);
      },true));
      ...
      globalThis.addEventListener('blur',()=>{camera.space=false;camera.touches.clear();camera.pinchDistance=0;finish();sync();});
      globalThis.addEventListener('resize',()=>{camera.space=false;camera.touches.clear();camera.pinchDistance=0;finish();sync();});
