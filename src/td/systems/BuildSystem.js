(function(ns){
  ...
  class BuildSystem{
    ...
    applyTowerSupport(){
      ...
      this.items.forEach(function(unit){if(unit.kind!=='unit')return;unit.towerSupportHaste=0;if(!unit.active){unit.supportHaste=0;return;}supports.forEach(function(source){const dx=unit.x-source.x,dy=unit.y-source.y;if(dx*dx+dy*dy<=source.radiusSquared)unit.towerSupportHaste=Math.max(unit.towerSupportHaste,source.haste);});unit.supportHaste=unit.towerSupportHaste;});
    }
