(function(ns){
  ...
  class BuildSystem{
    ...
    reset(){this.items=[];this.selected=null;this.pending=null;this.pointer={x:360,y:360,valid:false};this.unitRecovery={enabled:false,time:0,health:0,guard:0};}
    setUnitRecovery(policy){this.unitRecovery=Object.assign({enabled:false,time:0,health:0,guard:0},policy||{});this.items.filter(function(item){return item.kind==='unit';}).forEach(function(item){item.setRecoveryPolicy(this.unitRecovery);},this);return this.unitRecovery;}
    ...
    placeQueued(x,y,economy){const pending=this.pending;if(!pending)return false;const catalog=pending.kind==='building'?ns.config.buildings:ns.config.units;const cfg=catalog[pending.type];const cost=this.pendingCost();if(!cfg||!this.canPlaceAt(x,y,pending.kind)||!canAfford(economy,cost)||!spend(economy,cost))return false;const item=pending.kind==='building'?new ns.entities.Building(pending.type,x,y):new ns.entities.CombatUnit(pending.type,x,y);if(item.kind==='unit')item.setRecoveryPolicy(this.unitRecovery);item.mercenary=Boolean(pending.mercenary);item.totalSpent=cost.gold;this.items.push(item);this.selected=item;this.pending=null;return true;}
    upgrade(economy){if(!this.selected||this.selected.active===false)return false;const cost=this.selected.upgradeCost();if(!cost||!spend(economy,cost))return false;this.selected.upgrade();return true;}
    sell(economy){if(!this.selected||this.selected.active===false)return false;const removed=this.selected;removed.retired=true;refund(economy,removed.sellValue());if(typeof this.onRemove==='function')this.onRemove(removed);this.items=this.items.filter(function(item){return item!==removed;});this.selected=null;return true;}
    removeDefeated(){const defeated=this.items.filter(function(item){return item.kind==='unit'&&!item.active&&!item.recovering&&item.deathTime>=.8;});if(this.selected&&defeated.includes(this.selected))this.selected=null;if(typeof this.onRemove==='function')defeated.forEach(this.onRemove);this.items=this.items.filter(function(item){return !defeated.includes(item);});return defeated;}
    towers(){return this.items.slice();}
    combatUnits(){return this.items.filter(function(item){return item.kind==='unit'&&item.active;});}
    recoveringUnits(){return this.items.filter(function(item){return item.kind==='unit'&&item.recovering;});}
