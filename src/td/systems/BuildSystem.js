(function(ns){
  ...
  class BuildSystem{
    ...
    removeDefeated(){const defeated=this.items.filter(function(item){return item.kind==='unit'&&!item.active&&item.deathTime>=.8;});if(this.selected&&defeated.includes(this.selected))this.selected=null;if(typeof this.onRemove==='function')defeated.forEach(this.onRemove);this.items=this.items.filter(function(item){return !defeated.includes(item);});return defeated;}
