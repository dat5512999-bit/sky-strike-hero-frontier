(function(ns){
  ...
  class BuildSystem{
    ...
    sell(economy){if(!this.selected||this.selected.active===false)return false;this.selected.retired=true;refund(economy,this.selected.sellValue());this.items=this.items.filter(function(item){return item!==this.selected;},this);this.selected=null;return true;}
    removeDefeated(){const defeated=this.items.filter(function(item){return item.kind==='unit'&&!item.active&&item.deathTime>=.8;});if(this.selected&&defeated.includes(this.selected))this.selected=null;this.items=this.items.filter(function(item){return !defeated.includes(item);});return defeated;}
