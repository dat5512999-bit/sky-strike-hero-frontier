(function(ns){
  ...
  class LayoutSystem{
    ...
    preferenceKey(){return this.storageKey+(this.dimensions().coarse?':touch':':pointer');}
    restore(){let saved='auto';try{if(this.storage){const scoped=this.storage.getItem(this.preferenceKey());saved=scoped||this.storage.getItem(this.storageKey)||'auto';const view=this.dimensions();if(!scoped&&saved==='mobile'&&!view.coarse&&view.width>1100)saved='auto';}}catch(error){saved='auto';}this.choice=this.modes().includes(saved)?saved:'auto';this.apply();return this.choice;}
    select(mode){if(!this.modes().includes(mode))return false;this.choice=mode;try{if(this.storage){this.storage.setItem(this.preferenceKey(),mode);this.storage.setItem(this.storageKey,mode);}}catch(error){}this.apply();return true;}
