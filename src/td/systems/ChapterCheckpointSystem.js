(function(ns){
  ...
  class ChapterCheckpointSystem{
    ...
    capture(game){
      ...
      const unitKeys=['kind','type','x','y','guardX','guardY','targetStrategy','level','kills','strikes','gear','totalSpent','mercenary','unitId','health','maxHealth','active','cooldown'];
      const towerKeys=['kind','type','x','y','level','kills','shotsFired','branch','totalSpent','cooldown','retired'];
    ...
    restore(game,data){
      ...
      game.monsters=[];game.corpses=[];game.projectiles=[];game.summons=[];game.fieldLoot=[];game.build.applyTowerSupport();return true;
    }
  }
