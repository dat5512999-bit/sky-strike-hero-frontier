(function(ns){
  ...
  class BattleReportSystem{
    ...
    recordDefeat(target){if(!this.current)return;if(target&&target.kind==='unit'){if(target.recovering)this.current.unitDowns+=1;else this.current.unitLosses+=1;}else this.current.heroDowns+=1;}
    ...
    rank(record){if(!record.cleared)return'潰敗';if(record.gateDamage===0&&record.heroDowns===0&&record.unitDowns===0&&record.unitLosses===0)return'完美';if(record.gateDamage<=2&&record.unitDowns<=1&&record.unitLosses===0)return'穩固';if(record.gateDamage<=5&&record.unitLosses===0)return'苦戰';return'險守';}
    ...
    line(record){if(!record)return'尚無完成波次';return'第 '+record.wave+' 波 '+record.rank+'｜擊破 '+record.kills+'｜漏怪 '+record.leaks+'｜城門 -'+record.gateDamage+'｜英雄倒下 '+record.heroDowns+'｜守軍負傷 '+(record.unitDowns||0)+'｜永久損失 '+record.unitLosses+'｜+'+record.totalGold+'G';}
    finalLine(){const total=this.totals();return'總擊破 '+total.kills+'｜總漏怪 '+total.leaks+'｜城門損失 '+total.gateDamage+'｜英雄倒下 '+total.heroDowns+'｜守軍負傷 '+total.unitDowns+'｜永久損失 '+total.unitLosses+'｜戰鬥收入 '+total.gold+'G';}
  }
