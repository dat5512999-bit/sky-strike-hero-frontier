(function(ns){
  ...
  class BattleReportSystem{
    ...
    reset(){this.run={difficulty:'standard',difficultyName:'邊境守衛',profession:null,maxBaseHealth:20};this.waves=[];this.current=null;}
    beginRun(details){this.run=Object.assign({},this.run,details||{});}
    startWave(wave,snapshot){if(this.current&&this.current.wave===wave)return false;this.current={wave:wave,time:0,kills:0,killGold:0,leaks:0,gateDamage:0,heroDowns:0,unitLosses:0,baseBefore:snapshot.baseHealth,goldBefore:snapshot.gold,lumberBefore:snapshot.lumber};return true;}
    ...
    recordDefeat(target){if(!this.current)return;if(target&&target.kind==='unit')this.current.unitLosses+=1;else this.current.heroDowns+=1;}
    ...
    rank(record){if(!record.cleared)return'潰敗';if(record.gateDamage===0&&record.heroDowns===0&&record.unitLosses===0)return'完美';if(record.gateDamage<=2&&record.unitLosses===0)return'穩固';if(record.gateDamage<=5)return'苦戰';return'險守';}
    latest(){return this.waves[this.waves.length-1]||null;}
    totals(){return this.waves.reduce(function(total,wave){total.kills+=wave.kills;total.leaks+=wave.leaks;total.gateDamage+=wave.gateDamage;total.heroDowns+=wave.heroDowns;total.unitLosses+=wave.unitLosses;total.gold+=wave.totalGold;total.time+=wave.time;return total;},{kills:0,leaks:0,gateDamage:0,heroDowns:0,unitLosses:0,gold:0,time:0});}
    milestone(record){return Boolean(record&&record.cleared&&record.wave%5===0);}
    line(record){if(!record)return'尚無完成波次';return'第 '+record.wave+' 波 '+record.rank+'｜擊破 '+record.kills+'｜漏怪 '+record.leaks+'｜城門 -'+record.gateDamage+'｜英雄倒下 '+record.heroDowns+'｜守軍損失 '+record.unitLosses+'｜+'+record.totalGold+'G';}
    finalLine(){const total=this.totals();return'總擊破 '+total.kills+'｜總漏怪 '+total.leaks+'｜城門損失 '+total.gateDamage+'｜英雄倒下 '+total.heroDowns+'｜守軍損失 '+total.unitLosses+'｜戰鬥收入 '+total.gold+'G';}
  }
