(function(ns){
  ...
  class WaveSystem{
    ...
    composition(wave){
      ...
      const primary=['grunt','runner','shaman','brute'][(wave-1)%4];const result=[{type:primary,count:9+wave*2}];if(wave>=3)result.push({type:wave%2?'runner':'grunt',count:2+Math.ceil(wave/2)});return result;
    }
    ...
    update(dt,monsters){if(!this.active)return;this.spawnTimer-=dt;if(this.queue.length&&this.spawnTimer<=0){monsters.push(new ns.entities.Monster(this.queue.shift(),this.wave,ns.config.path));this.spawnTimer=this.wave%5===0?.72:Math.max(.26,.5-this.wave*.012);}if(!this.queue.length&&!monsters.some(function(m){return m.active;})){this.active=false;if(this.wave>=ns.config.totalWaves)this.complete=true;}}
