(function(ns){
  ...
  class EconomySystem{
    ...
    reset(){this.gold=ns.config.startGold;this.lumber=ns.config.startLumber;this.merit=0;this.totalEarned=0;this.totalSpent=0;}
    ...
    addKill(monster,bountyBonus){const gold=this.addGold(monster.reward*(1+(bountyBonus||0)));const merit=monster.type==='boss'?1:0;this.merit+=merit;return{gold:gold,merit:merit};}
    completeWave(reward){const gold=this.addGold(reward.gold);const lumber=Math.max(0,Math.round(reward.lumber||0));this.lumber+=lumber;return{gold:gold,lumber:lumber};}
