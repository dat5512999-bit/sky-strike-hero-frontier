(function(ns){
  ...
  class EconomySystem{
    ...
    reset(){this.gold=ns.config.startGold;this.lumber=ns.config.startLumber;this.merit=0;this.emblems=0;this.totalEarned=0;this.totalSpent=0;this.rewardRate=1;}
    setRewardRate(rate){this.rewardRate=Math.max(.3,Math.min(2,Number(rate)||1));return this.rewardRate;}
    canAfford(cost){return this.gold>=(cost.gold||0)&&this.lumber>=(cost.wood||cost.lumber||0)&&this.merit>=(cost.merit||0)&&this.emblems>=(cost.emblems||0);}
    spend(cost){if(!this.canAfford(cost))return false;const gold=cost.gold||0;this.gold-=gold;this.lumber-=cost.wood||cost.lumber||0;this.merit-=cost.merit||0;this.emblems-=cost.emblems||0;this.totalSpent+=gold;return true;}
    ...
    addEmblems(amount){const value=Math.max(0,Math.round(amount||0));this.emblems+=value;return value;}
