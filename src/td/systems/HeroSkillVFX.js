(function(ns){
  ...
  class HeroSkillVFX{
    ...
    static drawField(ctx,field){
      const trap=field.type==='hunter',chief=field.type==='chief',age=(trap?8:6)-field.time,fade=Math.min(1,field.time/.6),r=72;
      ctx.save();ctx.translate(field.x,field.y);ctx.globalAlpha=.55*fade;ctx.strokeStyle=trap?'#c5ad75':chief?'#ff9c4a':'#b07cda';ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);ctx.stroke();
