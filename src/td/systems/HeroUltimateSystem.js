(function(ns){
  ...
  class HeroUltimateSystem{
    ...
    static cast(hero,monsters,onKill,onHit){
      ...
      let target;
      if(hero.classType==='arcanist'||hero.classType==='naga'){
      ...
      const victims=monsters.filter(monster=>monster.active&&ns.utils.distance(center,monster)<=cfg.radius).sort((a,b)=>a===target?-1:b===target?1:b.progress()-a.progress()).slice(0,hero.classType==='rogue'?7:hero.classType==='naga'?10:12);
