(function(ns){
  ...
  class FrameTimingSystem{
    ...
    next(timestamp,speed,active=true){
      ...
      // The player-facing speed is literal: ×1 must not secretly simulate at ×2.
      // That doubled all collision, projectile and VFX work on the default setting.
      const rate=Math.max(1,Math.min(3,speed||1)),bounded=Math.min(elapsed,.1);
      ...
      // A 15 FPS frame at ×3 requests six small steps. Keep that pace possible
  ...
  class SpeedLoadGuard{
    ...
    reset(){this.effectiveSpeed=3;this.sampleMs=0;this.lostMs=0;this.slowFrameMs=0;this.stableMs=0;this.lastDiscardedMs=null;}
    ...
    observe(plan,requested,active){
      ...
      if(requested!==3||!active||plan.realMs>250){this.sampleMs=0;this.lostMs=0;this.slowFrameMs=0;this.stableMs=0;return false;}
      const elapsed=Math.max(0,Number(plan.realMs)||0);
      if(this.effectiveSpeed===3){
        this.sampleMs+=elapsed;this.lostMs+=delta;this.slowFrameMs=elapsed>=85?this.slowFrameMs+elapsed:Math.max(0,this.slowFrameMs-elapsed);
        if(this.sampleMs>=1500){
          if(this.lostMs>=250||this.slowFrameMs>=900)this.effectiveSpeed=2;
          this.sampleMs=0;this.lostMs=0;this.slowFrameMs=0;
        }
      }else{
        ...
        if(this.stableMs>=6000){this.effectiveSpeed=3;this.stableMs=0;this.slowFrameMs=0;}
      }
