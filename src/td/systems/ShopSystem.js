(function(ns){
  ...
  class ShopSystem{
    ...
    usable(id){const offer=ITEMS[id];if(!offer?.gear||!this.context)return true;const item=ns.systems.ArmorySystem.item(offer.gear),units=this.context.factions.available('unit');return Boolean(item&&((item.heroes||[]).includes(this.hero.classType)||(item.types||[]).some(id=>units.includes(id))));}
