(function(ns){
  ...
  const previewMobile=typeof location!=='undefined'&&new URLSearchParams(location.search).get('preview')==='iphone15promax';
  let layoutStorage=null;try{layoutStorage=previewMobile?null:globalThis.localStorage;}catch(error){}const layout=new ns.systems.LayoutSystem(document.body,layoutStorage,function(){return{width:globalThis.innerWidth,height:globalThis.innerHeight,coarse:previewMobile||(globalThis.matchMedia?globalThis.matchMedia('(pointer: coarse)').matches:false)};});
  ...
  ui.performanceToggle=document.getElementById('td-performance-toggle');
  ui.performancePanel=document.getElementById('td-performance-panel');
  ...
  if(globalThis.towerFrontierGame.feedback)globalThis.towerFrontierGame.feedback.announcer=document.getElementById('td-battle-announcement');
  const accessibilityOptions=[['td-access-contrast','contrast','off'],['td-access-text','text','off'],['td-access-effects','effects',globalThis.matchMedia?.('(prefers-reduced-motion: reduce)').matches?'reduced':'normal']];
  accessibilityOptions.forEach(([id,name,defaultValue])=>{
    const control=document.getElementById(id),key='towerFrontierAccess'+name;let value=defaultValue;
    try{value=layoutStorage?.getItem(key)||defaultValue;}catch(error){}
    const enabled=value==='on'||value==='reduced';control.checked=enabled;
    const apply=()=>{const next=control.checked?(name==='effects'?'reduced':'on'):(name==='effects'?'normal':'off');document.body.dataset['access'+name[0].toUpperCase()+name.slice(1)]=next;if(name==='effects'&&globalThis.towerFrontierGame?.feedback)globalThis.towerFrontierGame.feedback.reducedFx=control.checked;try{layoutStorage?.setItem(key,next);}catch(error){}};
    apply();control.addEventListener('change',apply);
  });
  globalThis.addEventListener('change',event=>{const option=event.target?.dataset?.accessOption;if(!option)return;const control=document.getElementById('td-access-'+option);if(!control)return;control.checked=event.target.checked;control.dispatchEvent(new Event('change'));});
