(function(ns){
  'use strict';
  const CELL_SIZE=64,DEFAULT_MAP_ID='emberroad';
  ...
  function terrainState(map,x,y){
    if(x<42||x>map.width-42||y<62||y>map.height-46)return 'restricted';
    const routes=map.routes||[map.path];
    if(routes.some(route=>route.some((point,index,path)=>index&&segmentDistance(x,y,path[index-1],point)<(map.roadClearance||55))))return 'restricted';
    const footprint=offsets(map);
    if((map.blockedAreas||[]).some(area=>footprint.some(([dx,dy])=>pointInPolygon(x+dx,y+dy,area))))return 'restricted';
  ...
  function selectionPayload(map,selected){const available=new Map(cells(map).map(cell=>[cell.key,cell]));const buildable=[...selected].filter(id=>available.has(id)).map(id=>{const cell=available.get(id);return {column:cell.column,row:cell.row,x:cell.x,y:cell.y,originalState:cell.state,requiresReview:cell.state==='restricted'};}).sort((a,b)=>a.row-b.row||a.column-b.column);return {format:'hero-frontier-grid-v2',map:map.id,cellSize:CELL_SIZE,buildable};}
  ns.mapTools=ns.mapTools||{};
  const api=Object.freeze({CELL_SIZE,DEFAULT_MAP_ID,key,cells,baseline,selectionPayload,terrainState});
  ns.mapTools.MapGrid=api;
  // Keep the initial 2-2 entry point compatible while the editor now supports every released map.
  ns.mapTools.EmberroadGrid=api;
