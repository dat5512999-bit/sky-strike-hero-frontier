(function(ns){
  ...
  class Summon{
    ...
    draw(ctx,art){if(art&&art.drawSummon&&art.drawSummon(ctx,this))return;if(this.form==='rogue'&&art){ctx.save();ctx.globalAlpha=.65;art.drawCombatUnit(ctx,{type:'rogue',x:this.x,y:this.y,level:1,state:this.state,frame:this.frame,facing:this.facing,config:function(){return{color:'#c884df'};}});ctx.restore();return;}ctx.save();ctx.translate(this.x,this.y);ctx.globalAlpha=Math.min(.85,this.time/2);ctx.strokeStyle=this.color;ctx.fillStyle=this.color;ctx.lineWidth=2;ctx.beginPath();ctx.ellipse(0,8,17,6,0,0,Math.PI*2);ctx.stroke();ctx.rotate(this.clock*2);for(let i=0;i<3;i++){ctx.rotate(Math.PI*2/3);ctx.beginPath();ctx.arc(13,0,3,0,Math.PI*2);ctx.fill();}ctx.rotate(-this.clock*2);ctx.globalAlpha*=.45;ctx.beginPath();ctx.arc(0,-8,10+Math.sin(this.clock*5)*2,0,Math.PI*2);ctx.fill();ctx.restore();}
  }
