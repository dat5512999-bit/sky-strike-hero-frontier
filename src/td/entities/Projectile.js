(function(ns){
  ...
  class Projectile{
    ...
    hit(monsters,onKill){const self=this;const victims=this.splash?monsters.filter(function(m){return m.active&&ns.utils.distance(m,self.target)<=self.splash;}):[this.target];victims.forEach(function(monster){if(self.slow)monster.applySlow(self.slow,self.slowTime);if(monster.takeDamage(self.damage)&&onKill)onKill(monster);});}
