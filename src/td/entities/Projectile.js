(function(ns){
  ...
  class Projectile{
    ...
    update(dt,monsters,onKill,onHit){
      ...
      if(distance<=move+target.radius){this.hit(monsters,onKill,onHit);this.active=Boolean(this.chain||this.splash||this.style==='claw'||this.style.startsWith('goblin-'));return;}this.x+=(target.x-this.x)/distance*move;this.y+=(target.y-this.y)/distance*move;
    }
