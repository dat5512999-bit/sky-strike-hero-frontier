(function(ns){
  ...
  class Monster{
    constructor(type,wave,path){
      ...
      this.health=Math.round(this.health*scale);this.maxHealth=this.health;this.displayHealth=this.health;this.reward=Math.round(this.reward*(1+(wave-1)*.025));this.active=true;this.leaked=false;this.slowFactor=1;this.slowTimer=0;this.hitFlash=0;this.walkDistance=0;this.frame=0;this.state='walk';this.facing=0;this.hitTime=0;this.deathTime=0;this.attackCooldown=.6;this.attackWindup=0;this.combatTarget=null;this.abilityCooldown=this.type==='boss'?5:0;this.abilityWindup=0;this.abilityRadius=126;this.bossPhase=1;this.phaseTriggered={2:false,3:false};this.enraged=false;
    }
    ...
    takeDamage(amount){if(!this.active)return false;this.health-=Math.max(1,amount-(this.armor||0));this.hitFlash=.1;if(this.hitTime<=0){this.hitTime=.24;this.state='hit';this.frame=0;}if(this.health<=0){this.health=0;this.active=false;this.state='death';this.deathTime=0;this.frame=0;return true;}return false;}
