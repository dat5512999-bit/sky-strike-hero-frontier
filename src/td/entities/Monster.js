(function(ns){
  ...
  const TYPES={
    ...
    warder:{name:'戰紋盾衛',color:'#72a7d8',speed:46,health:158,reward:23,radius:20,baseDamage:2,armor:2,armorType:'heavy',barrier:72,visualBase:'brute'},
    healer:{name:'血契祭司',color:'#72d58b',speed:53,health:112,reward:22,radius:16,baseDamage:1,armorType:'arcane',visualBase:'grunt'},
    commander:{name:'軍團旗手',color:'#e49755',speed:48,health:176,reward:28,radius:18,baseDamage:2,armor:1,armorType:'light',visualBase:'grunt'},
  ...
  class Monster{
    constructor(type,wave,path,modifiers){
      const preset=TYPES[type]||TYPES.grunt,mods=modifiers||{};const scale=1+(wave-1)*.105,healthRate=mods.enemyHealth||1;
      ...
      this.modifiers=Object.assign({},mods);this.health=Math.round(this.health*scale*healthRate);this.maxHealth=this.health;this.displayHealth=this.health;this.barrier=Math.round((this.barrier||0)*scale*healthRate);this.maxBarrier=this.barrier;this.reward=Math.round(this.reward*(1+(wave-1)*.025));this.speed*=mods.enemySpeed||1;this.damageMultiplier=mods.enemyDamage||1;this.baseDamage=Math.max(1,Math.round(this.baseDamage*(mods.enemyDamage||1)));this.active=true;this.leaked=false;this.slowFactor=1;this.slowTimer=0;this.moveAura=1;this.temporaryArmor=0;this.hitFlash=0;this.walkDistance=0;this.frame=0;this.state='walk';this.facing=0;this.hitTime=0;this.deathTime=0;this.attackCooldown=.6;this.attackWindup=0;this.combatTarget=null;this.abilityCooldown=this.type==='boss'?5:0;this.abilityWindup=0;this.abilityRadius=126;this.bossPhase=1;this.phaseTriggered={2:false,3:false};this.enraged=false;this.traitCooldown=1.4;
    }
    ...
    draw(ctx,art){
      ...
      const barY=this.y-({grunt:82,runner:76,brute:102,warder:102,healer:82,commander:88}[this.type]||this.radius+18),barRatio=Math.max(0,this.displayHealth/this.maxHealth);ctx.shadowBlur=0;ctx.fillStyle='rgba(8,7,12,.82)';ctx.fillRect(this.x-this.radius,barY,this.radius*2,5);ctx.fillStyle='#701f24';ctx.fillRect(this.x-this.radius,barY,this.radius*2*(this.health/this.maxHealth),5);ctx.fillStyle=this.color;ctx.globalAlpha=.72;ctx.fillRect(this.x-this.radius,barY,this.radius*2*barRatio,5);if(this.maxBarrier){ctx.globalAlpha=1;ctx.fillStyle='#70c9ff';ctx.fillRect(this.x-this.radius,barY-3,this.radius*2*Math.max(0,this.barrier/this.maxBarrier),2);}ctx.restore();
    }
  }
