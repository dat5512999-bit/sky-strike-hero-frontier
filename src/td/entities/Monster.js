(function(ns){
  ...
  const TYPES={
    ...
    ,orcMirrorGuard:{name:'鏡甲裂牙衛',color:'#b980a9',speed:45,health:228,reward:35,radius:21,baseDamage:2,armor:3,armorType:'heavy',mirrorStagger:{chance:.24,duration:.82,lockout:2.8},visualBase:'orcBanner'}
    ...
    ,frostSkyraider:{name:'霜翼掠襲者',color:'#d4f6ff',speed:106,health:118,reward:30,radius:17,baseDamage:2,armorType:'light',flying:true,visualBase:'frostRimeStalker'}
    ,nagaPhasewarden:{name:'相位潮衛',color:'#75efe2',speed:50,health:184,reward:34,radius:19,baseDamage:2,armor:2,armorType:'arcane',immuneAttackTypes:['pierce'],visualBase:'nagaSiltImp'}
  };
  class Monster{
    constructor(type,wave,path,modifiers){
      const preset=TYPES[type]||TYPES.grunt,mods=modifiers||{};const earlyWaves=Math.min(Math.max(0,wave-1),14),lateWaves=Math.max(0,wave-15),scale=1+earlyWaves*.105+lateWaves*.07;
      ...
      // Bosses need their own milestone curve: player armies gain equipment,
      // evolutions and synergy much faster than a generic per-wave HP slope.
      const bossTier=this.combatRole==='boss'?Math.max(1,Math.ceil(wave/5)):0,bossHealth=bossTier?1+(bossTier-1)*.28+Math.max(0,bossTier-4)*.10:1,healthRate=(mods.enemyHealth||1)*(1+(mods.healthGrowth||0)*Math.max(0,wave-1))*bossHealth;
      this.modifiers=Object.assign({},mods);this.bossTier=bossTier;this.bossHealthMultiplier=bossHealth;this.health=Math.round(this.health*scale*healthRate);this.maxHealth=this.health;this.displayHealth=this.health;this.barrier=Math.round((this.barrier||0)*scale*healthRate);this.maxBarrier=this.barrier;if(bossTier)this.armor=(this.armor||0)+Math.floor(Math.max(0,wave-5)/10);this.reward=Math.max(1,Math.round(this.reward*(1+(wave-1)*.025)*(mods.bountyScale||1)));this.speed*=mods.enemySpeed||1;this.damageMultiplier=mods.enemyDamage||1;this.baseDamage=Math.max(1,Math.round(this.baseDamage*(mods.enemyDamage||1)));this.active=true;this.leaked=false;this.slowFactor=1;this.slowTimer=0;this.moveAura=1;this.temporaryArmor=0;this.controlResistance=0;this.bossWardTime=0;this.revealed=!this.concealed;this.revealPulse=0;this.immuneFlash=0;this.hitFlash=0;this.hitKickX=0;this.hitKickY=0;this.hitKickDuration=0;this.walkDistance=0;this.walkPhase=0;this.frame=0;this.state='walk';this.facing=0;this.visualHeading=0;this.routeHeading=0;this.hitTime=0;this.deathTime=0;this.attackCooldown=.6;this.attackWindup=0;this.attackVisualTime=0;this.attackVisualDuration=0;this.attackVisualRelease=0;this.combatTarget=null;this.abilityCooldown=this.combatRole==='boss'?5:0;this.abilityWindup=0;this.abilityRadius=126+(this.combatRole==='boss'?Math.max(0,Math.floor(wave/5)-3)*4:0);this.bossAspect=type==='boss'?({35:'tide',40:'siege',45:'frost'}[wave]||'war'):type==='nagaSiltwaterDemonlord'?'demonlord':null;this.bossPhase=1;this.phaseTriggered={2:false,3:false};this.enraged=false;this.traitCooldown=1.4;
    ...
    update(dt,hero,defenders,maxAdvance){
      if(!this.active)return;ns.systems.HeroEvolutionCombat?.updateEnemy(this,dt);this.displayHealth+=(this.health-this.displayHealth)*Math.min(1,dt*11);this.hitTime=Math.max(0,this.hitTime-dt);this.hitFlash=Math.max(0,this.hitFlash-dt);this.immuneFlash=Math.max(0,this.immuneFlash-dt);this.revealPulse=Math.max(0,this.revealPulse-dt);this.bossWardTime=Math.max(0,this.bossWardTime-dt);this.criticalFlash=Math.max(0,(this.criticalFlash||0)-dt);this.hitKickTime=Math.max(0,(this.hitKickTime||0)-dt);if(this.hitKickTime<=0){this.hitKickX=0;this.hitKickY=0;}this.slowTimer=Math.max(0,this.slowTimer-dt);if(this.slowTimer<=0)this.slowFactor=1;
    ...
    canBeDamagedBy(source,projectile){const canHitAir=projectile&&projectile.canHitAir!==undefined?projectile.canHitAir:source&&source.canHitAir;return !this.flying||Boolean(canHitAir);}
    isTargetableBy(source){return(!this.concealed||this.revealed)&&this.canBeDamagedBy(source);}
    reveal(){if(!this.concealed||this.revealed)return false;this.revealed=true;this.revealPulse=.65;return true;}
    takeDamage(amount,source){if(!this.active)return false;if(this.immuneAttackTypes&&this.immuneAttackTypes.includes(source&&source.attackType)){this.immuneFlash=.2;return false;}if(this.concealed)this.reveal();const vulnerable=this.vulnerability&&(!source||source.owner!==this.vulnerability.owner)?this.vulnerability.rate:0,rate=1+(this.frostStatus?.window>0&&this.frostStatus.deep?(ns.config.frostRules?.deepVulnerability||0):0)+vulnerable+(this.shockTime>0?.1:0),armor=Math.max(0,(this.armor||0)+(this.temporaryArmor||0)+(this.mapPressure?.armorBonus||0)-(this.evolutionArmorBreak||0)-((source&&source.armorPierce)||0));let damage=Math.max(1,amount*rate*(ns.systems.HeroEvolutionCombat?.damageRate(this,source)||1)-armor);if(this.barrier>0){const absorbed=Math.min(this.barrier,damage);this.barrier-=absorbed;damage-=absorbed;}if(damage>0)this.health-=damage;this.hitFlash=.1;if(this.hitTime<=0){this.hitTime=.24;this.state='hit';this.frame=0;}if(this.health<=0){this.health=0;this.active=false;this.state='death';this.deathTime=0;this.frame=0;return true;}return false;}
    applySlow(factor,time){const resistance=Math.max(this.controlResistance||0,this.bossWardTime>0?.35:0),adjusted=resistance>0?1-(1-factor)*(1-resistance):factor;this.slowFactor=Math.min(this.slowFactor,adjusted);this.slowTimer=Math.max(this.slowTimer,time);}
    ...
    draw(ctx,art){
      ...
      ctx.save();if(this.state==='death')ctx.globalAlpha=Math.max(0,Math.min(1,(1-this.deathTime)/.35));if(this.concealed&&!this.revealed)ctx.globalAlpha*=.3;
      drawBody();
      ...
      if(this.flying){ctx.save();ctx.globalAlpha=.72;ctx.strokeStyle='#d7faff';ctx.lineWidth=2;ctx.setLineDash([4,3]);ctx.beginPath();ctx.ellipse(this.x,this.y+13,this.radius+9,5,0,0,Math.PI*2);ctx.stroke();ctx.restore();}if(this.concealed&&!this.revealed){ctx.save();ctx.globalAlpha=.7;ctx.strokeStyle='#9ef3c2';ctx.setLineDash([3,4]);ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(this.x,this.y-8,this.radius+5,0,Math.PI*2);ctx.stroke();ctx.restore();}if(this.immuneAttackTypes){ctx.save();ctx.globalAlpha=.55+.35*Math.sin((this.immuneFlash||0)*50);ctx.strokeStyle='#74fff0';ctx.lineWidth=2.5;ctx.beginPath();ctx.arc(this.x,this.y-7,this.radius+8,0,Math.PI*2);ctx.stroke();ctx.fillStyle='#b7fffb';ctx.font='900 12px Segoe UI';ctx.textAlign='center';ctx.fillText('穿刺免疫',this.x,this.y-this.radius-20);ctx.restore();}if(this.mirrorStagger){ctx.save();ctx.globalAlpha=.7;ctx.strokeStyle='#efb8ff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(this.x,this.y-7,this.radius+7,0,Math.PI*2);ctx.stroke();ctx.fillStyle='#fff0ff';ctx.font='900 14px Segoe UI';ctx.textAlign='center';ctx.fillText('✦',this.x,this.y-this.radius-18);ctx.restore();}ctx.restore();ctx.restore();
    }
  }
  const drawBase=Monster.prototype.draw;
  Monster.prototype.draw=function(ctx,art){
    drawBase.call(this,ctx,art);
    ns.systems.HeroEvolutionPresentation?.drawEnemyMark(ctx,this);
  ...
  Monster.prototype.takeDamage=function(amount,source){if(this.evolutionTrial&&source?.owner!==this.trialOwner)return false;return takeMonsterDamage.call(this,amount,source);};
