(function(ns){
  'use strict';
  class Hero{
    constructor(x,y){const cfg=ns.config.hero;Object.assign(this,{x:x,y:y,targetX:x,targetY:y,health:cfg.maxHealth,maxHealth:cfg.maxHealth,cooldown:0,novaCooldown:0,facing:0,active:true,state:'idle',frame:0,animationTime:0,attackTimer:0,castTimer:0,pendingTarget:null,selected:false,classType:'arcanist',fields:[],equipment:{spear:0,rune:0,charm:0},skillCooldowns:{thunder:0,summon:0}});}
    chooseClass(type){if(!ns.systems.HeroRoster.CLASSES[type])return false;this.classType=type;return true;}
    combatConfig(){const base=ns.config.hero,power=1+this.equipment.spear*.2;return Object.assign({},base,ns.systems.HeroRoster.get(this.classType),{damage:ns.systems.HeroRoster.get(this.classType).damage*power,novaDamage:base.novaDamage*power,novaCooldown:base.novaCooldown*(1-this.equipment.rune*.1)});}
    beginCast(){this.castTimer=.56;this.attackTimer=0;this.pendingTarget=null;this.setState('cast');}
    castThunder(monsters,onKill,projectiles,onHit){if(this.classType!=='arcanist')return ns.systems.HeroRoster.cast(this,1,monsters,onKill,onHit);if(this.skillCooldowns.thunder>0)return false;const targets=monsters.filter(function(m){return m.active&&ns.utils.distance(this,m)<=220;},this).sort(function(a,b){return b.progress()-a.progress();}).slice(0,4);if(!targets.length)return false;const shot=new ns.entities.Projectile(this,targets[0],{damage:55*(1+this.equipment.spear*.2),color:'#b9eaff',attackType:'magic',chain:4,chainRange:110});shot.hit(monsters,onKill,onHit);if(projectiles)projectiles.push(shot);this.skillCooldowns.thunder=12*(1-this.equipment.rune*.1);this.beginCast();return true;}
    castSummon(summons){if(this.skillCooldowns.summon>0)return false;const cap=2+(this.equipment.charm>=3?1:0),existing=summons.filter(function(unit){return unit.active&&unit.owner===this;},this).length;if(existing>=cap)return false;for(let i=existing;i<cap;i++)summons.push(new ns.entities.Summon(this,{offsetX:(i-(cap-1)/2)*42,duration:12+this.equipment.charm*3,damage:14*(1+this.equipment.charm*.2),color:ns.systems.HeroRoster.get(this.classType).color,form:this.classType,range:this.classType==='arcanist'?120:45,speed:this.classType==='hunter'?160:140}));this.skillCooldowns.summon=22*(1-this.equipment.rune*.1);this.beginCast();return true;}
    ...
    castNova(monsters,onKill,onHit){if(this.classType!=='arcanist')return ns.systems.HeroRoster.cast(this,0,monsters,onKill,onHit);const cfg=this.combatConfig();if(this.novaCooldown>0)return false;monsters.forEach(function(monster){if(monster.active&&ns.utils.distance(this,monster)<=cfg.novaRange){monster.applySlow(.38,2.4);const before=monster.health,killed=monster.takeDamage(cfg.novaDamage);if(onHit)onHit(monster,before-monster.health,false,{color:'#70e7ff',owner:this});if(killed&&onKill)onKill(monster);}},this);this.novaCooldown=cfg.novaCooldown;this.castTimer=.56;this.attackTimer=0;this.pendingTarget=null;this.setState('cast');return true;}
    draw(ctx,art){
