(function(ns){
  ...
  class Hero{
    ...
    castNova(monsters,onKill){if(this.classType!=='arcanist')return ns.systems.HeroRoster.cast(this,0,monsters,onKill);const cfg=this.combatConfig();if(this.novaCooldown>0)return false;monsters.forEach(function(monster){if(monster.active&&ns.utils.distance(this,monster)<=cfg.novaRange){monster.applySlow(.38,2.4);if(monster.takeDamage(cfg.novaDamage)&&onKill)onKill(monster);}},this);this.novaCooldown=cfg.novaCooldown;this.castTimer=.56;this.attackTimer=0;this.pendingTarget=null;this.setState('cast');return true;}
    draw(ctx,art){
