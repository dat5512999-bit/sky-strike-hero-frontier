(function(ns){
  ...
  class Hero{
    ...
    draw(ctx){ctx.save();ctx.translate(this.x,this.y);ctx.shadowColor='#65efb2';ctx.shadowBlur=16;ctx.fillStyle='rgba(0,0,0,.35)';ctx.beginPath();ctx.ellipse(0,15,21,10,0,0,Math.PI*2);ctx.fill();ctx.rotate(this.facing+Math.PI/2);ctx.fillStyle='#e8ddbd';ctx.beginPath();ctx.moveTo(0,-25);ctx.lineTo(18,18);ctx.lineTo(0,10);ctx.lineTo(-18,18);ctx.closePath();ctx.fill();ctx.fillStyle='#24866d';ctx.beginPath();ctx.arc(0,-4,12,0,Math.PI*2);ctx.fill();ctx.fillStyle='#8fffc4';ctx.fillRect(-3,-33,6,24);ctx.restore();
