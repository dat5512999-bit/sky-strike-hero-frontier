(function(ns){
  ...
  class FrontierApp{
    ...
    lockReason(kind,id){const source=ns.systems.StoryCatalog.missions.find(m=>m.rewards?.[kind]?.includes(id));if(source)return '完成「'+source.name+'」';if(kind==='maps')return '後續故事尚未開放';if(kind==='difficulties')return '後續挑戰尚未開放';if(id==='frostland')return '霜原故事任務尚未開放';if(id==='goblin')return '地精故事任務尚未開放';if(id==='wild'||id==='chief')return '第二章對應任務尚未開放';return '對應故事任務尚未開放';}
    async startCinematic(){
      ...
      if(progress.prologueSeen)return this.startStoryCards();
      this.storyStarting=true;
    ...
    startStoryCards(selected,replay=false){
    ...
    finishStoryCards(){
      if(this.storyCardReplay)return this.show('story');
      return this.launchStory(this.storyCardMission||ns.systems.StoryCatalog.mission);
    }
    renderStoryCards(){
    ...
    localizeInterface(){
      const terms=[
        ['HERO FRONTIER · CHAPTER I','英雄邊境 · 第一章'],['HERO FRONTIER · PLAYER','英雄邊境 · 玩家資料'],['HERO FRONTIER','英雄邊境'],
        ['Different Heroes, One World','不同的英雄，同一個世界'],['Different Heroes','不同的英雄'],['One World','同一個世界'],
        ['Cinematic Collection','劇情典藏'],['STORY MODE','劇情模式'],['EXPEDITION','自由遠征'],['PLAYER PROFILES','玩家資料'],
        ['NEW TEST ROUND','新測試輪次'],['ACCOUNT WALLET','帳戶錢包'],['COMING NEXT','即將推出'],['CONFIRMED / STORY EXPERIENCE','已確認／故事體驗'],['Replay Cinematic','重播影片'],['CHAPTER I','第一章'],
        ['HOME','首頁'],['CODEX','世界圖鑑'],['SHOP','商城'],['RANKING','排行榜'],['FRIENDS','好友'],['SETTINGS','設定'],['HERO','英雄'],['FRONTIER','邊境'],['ROUND','輪次']
      ];
      const walker=document.createTreeWalker(this.root,NodeFilter.SHOW_TEXT);
      for(let node=walker.nextNode();node;node=walker.nextNode()){
        let text=node.nodeValue;
        terms.forEach(([from,to])=>{text=text.split(from).join(to);});
        text=text.replace(/CHAPTER\s+(\d+)/g,'第 $1 章');
        node.nodeValue=text;
      }
    }
    ...
    render(){const p=this.store.current(),b=this.button.bind(this),m=ns.systems.StoryCatalog.mission;const label=p.kind==='admin'?'管理者遊玩 · 全內容可用':'一般玩家 · 第 '+p.round+' 輪';const nav=[['home','首頁'],['story','劇情模式'],['free','自由遠征'],['codex','世界圖鑑'],['shop','商城'],['ranking','排行榜'],['friends','好友'],['settings','設定']];
      ...
      else if(this.page==='chapter-start')content='<article class="cinematic-collection"><small>HERO FRONTIER · CHAPTER I</small><h1>《破碎的和平》</h1><p>序章已收錄至世界圖鑑 → 編年史 → Cinematic Collection。</p><p>第一個故事任務：烽火初燃</p>'+b('chapter-battle','觀看戰前短劇情',null,'gold')+b('story','返回章節選單')+'</article>';
      ...
      else if(this.page==='story-cards')content=this.renderStoryCards();
      ...
      else if(this.page==='settings'){const test=this.store.state.profiles.test;content='<div class="section-heading"><small>PLAYER PROFILES</small><h1>玩家進度與管理者檔案</h1><p>目前：'+label+'。不同檔案的進度、裝備、戰績與中途存檔皆獨立。</p></div><div class="settings-grid"><article class="journey-card"><h2>管理者遊玩</h2><p>保留原有紀錄，可使用所有英雄、軍團、地圖與難度。故事仍記錄實際通關進度。</p>'+b('admin','切換管理者')+'</article><article class="journey-card"><h2>一般玩家進度</h2><p>'+(test?'現有第 '+test.round+' 輪 · '+escape(test.startedVersion):'從王國與雷恩開始，體驗正常解鎖流程。')+'</p>'+ (test?b('test','繼續此輪遊玩'):'')+b('new-round','開始新一輪遊玩',null,'gold')+'</article><article class="journey-card"><h2>備份與還原</h2><p>備份包含管理者、一般玩家與封存輪次。匯入會先保存目前資料。</p>'+b('export','匯出完整備份')+'<label class="import-label">匯入備份<input type="file" id="profile-import" accept="application/json"></label></article><article class="journey-card"><h2>封存進度</h2><p>每次開新一輪都保留上一輪，可回復繼續遊玩。</p>'+ (this.store.state.archives.map(a=>b('restore','回復 ROUND '+a.round+' · '+escape(a.startedVersion),String(a.round))).join('')||'<p>尚無封存紀錄</p>')+'</article><article class="journey-card"><h2>遊戲程式更新</h2><p>畫面仍是舊版時，重新取得新版程式；保留進度、購買紀錄與裝備。</p><a class="import-label" href="update.html" target="_top">更新並重新開啟</a></article></div>';}
      else if(this.page==='confirm-round'){content='<article class="confirm-card"><small>NEW TEST ROUND</small><h1>從全新玩家重新開始</h1><p>目前玩家進度會封存。新一輪將重設故事、解鎖、戰績與中途存檔；管理者紀錄完整保留。</p>'+b('settings','取消')+b('confirm-round','封存並開始新一輪',null,'gold')+'</article>';}
      ...
      if(this.page==='settings'){const card='<article class="journey-card td-access-card"><h2>畫面與動態</h2><fieldset class="td-access-controls"><legend>辨識設定</legend><label><input type="checkbox" data-access-option="contrast" '+(document.body.dataset.accessContrast==='on'?'checked':'')+'> 提高文字與按鈕對比</label><label><input type="checkbox" data-access-option="text" '+(document.body.dataset.accessText==='on'?'checked':'')+'> 放大戰鬥資訊文字</label><label><input type="checkbox" data-access-option="effects" '+(document.body.dataset.accessEffects==='reduced'?'checked':'')+'> 減少裝飾特效與動態</label><small>設定在大廳與戰場共用，並保存在此裝置。</small></fieldset></article>';content=content.replace(/<\/div>$/,card+'</div>');}
      this.root.dataset.page=this.page;
      this.root.innerHTML='<aside class="lobby-nav"><div class="lobby-logo"><span>HERO</span><strong>FRONTIER</strong><small>─ 英雄的邊境 ─</small></div><nav>'+nav.map(([id,name])=>b(id,this.icon(id)+'<span>'+name+'<small>'+({home:'HOME',story:'STORY MODE',free:'EXPEDITION',codex:'CODEX',shop:'SHOP',ranking:'RANKING',friends:'FRIENDS',settings:'SETTINGS'}[id])+'</small></span>',null,this.page===id?'active':'')).join('')+'</nav><small class="nav-motto">Different Heroes<br>One World</small></aside><div class="lobby-main"><header class="lobby-top"><button type="button" class="profile-badge" data-action="profile" aria-label="展開個人頁籤"><img src="assets/td/opening/hero-hunter-selection-v1.png" alt=""><span>'+label+'<small>HERO FRONTIER · v0.85.39</small></span></button><div class="wallet-bar">'+this.walletButton('gold',p.wallet.gold)+this.walletButton('diamonds',p.wallet.diamonds)+'</div><div class="header-actions">'+(this.page!=='home'?b('home',this.icon('home')+'<span>返回大廳</span>'):'')+b('settings',this.icon('settings')+'<span>設定與玩家資料</span>')+'</div></header><main class="lobby-content">'+(this.message?'<p class="app-message" role="alert">'+escape(this.message)+'</p>':'')+content+'</main></div>';
    ...
    act(action,value){if(this.storyStarting||this.shopOpening)return;this.message='';if(action==='story-chapter'){const first=ns.systems.StoryCatalog.missions.find(m=>m.chapter===Number(value));if(first)this.selectedStoryMission=first.id;return this.render();}if(action==='story-select'){this.selectedStoryMission=value;return this.render();}if(action==='story-battle'){const mission=ns.systems.StoryCatalog.getMission(value);return mission&&this.startStoryCards(mission);}if(action==='story-card-replay'){const mission=ns.systems.StoryCatalog.getMission(value)||ns.systems.StoryCatalog.getMission(this.selectedStoryMission)||ns.systems.StoryCatalog.mission;return this.startStoryCards(mission,true);}if(action==='story-card-skip')return this.finishStoryCards();if(action==='story-card-next'){const cards=(this.storyCardMission||ns.systems.StoryCatalog.mission).storyCards;if(this.storyCardIndex>=cards.length-1)return this.finishStoryCards();this.storyCardIndex++;this.render();this.root.querySelector('.story-card h1')?.focus();return;}if(action==='profile-art')return this.run(()=>{this.store.setProfileArt(value);this.render();});if(action==='retry-prologue')return this.finishPrologue();if(action==='chapter-battle')return this.startStoryCards(ns.systems.StoryCatalog.mission);if(action==='shop')return this.openShop();if(action==='codex'){location.href='codex.html';return;}if(action==='free')return this.openFree();if(action==='home')return this.home();if(action==='story'){this.replayOnly=false;return this.show('story');}if(['profile','settings','codex','shop','ranking','friends','wallet','skin-preview','limited-preview'].includes(action))return this.show(action);if(action==='new-round')return this.show('confirm-round');if(action==='confirm-round')return this.run(()=>{this.store.newRound();this.home();});if(action==='admin'||action==='test')return this.run(()=>{this.store.switchTo(action);this.home();});if(action==='restore')return this.run(()=>{this.store.restoreRound(Number(value));this.home();});if(action==='export')return this.download(this.store.export(),'hero-frontier-profiles.json');if(action==='cinematic'||action==='replay'){this.replayOnly=action==='replay';return this.startCinematic();}if(action==='skip'||action==='next'&&this.slide===2)return this.replayOnly?this.show('story'):this.launchStory();if(action==='next'){this.slide++;return this.render();}if(action==='again'){const run=this.store.current().lastFreeRun;return run?.mode==='story'?this.startCinematic():this.openFree(run);}if(action==='continue'){this.openFree();this.game.continueExpedition();}}
  ...
  // Chapter-aware campaign shell: a chapter is selected by the mission rather
  ...
  FrontierApp.prototype.render=function(){renderWithRelease.call(this);const release=this.root.querySelector('.profile-badge small');if(release)release.textContent='HERO FRONTIER · v0.85.49';};
