(function(ns){
  ...
  class FrontierApp{
    ...
    startStoryCards(replay=false){
      this.storyCardIndex=0;
      this.storyCardReplay=Boolean(replay);
    ...
    finishStoryCards(){
      ...
      return this.launchStory(ns.systems.StoryCatalog.mission);
    }
    renderStoryCards(){
      const cards=ns.systems.StoryCatalog.mission.storyCards,card=cards[this.storyCardIndex]||cards[0],b=this.button.bind(this),last=this.storyCardIndex===cards.length-1;
      return '<article class="story-card" aria-label="烽火初燃戰前劇情"><div class="story-card-stage story-card-stage-'+this.storyCardIndex+'"><img src="'+escape(card.image)+'" alt="" style="object-position:'+escape(card.focus)+'"><div class="story-card-shade"></div><div class="story-card-copy"><small>CHAPTER I · '+escape(card.label)+'</small><h1>'+escape(card.title)+'</h1><p>'+escape(card.line)+'</p></div></div><div class="story-card-bar"><span aria-label="第 '+(this.storyCardIndex+1)+' 張，共 '+cards.length+' 張">'+(this.storyCardIndex+1)+' / '+cards.length+'</span><div class="story-card-dots" aria-hidden="true">'+cards.map((_,i)=>'<i'+(i===this.storyCardIndex?' class="active"':'')+'></i>').join('')+'</div><div class="story-card-actions">'+b('story-card-skip',this.storyCardReplay?'返回章節':'跳過並出征')+b('story-card-next',last?(this.storyCardReplay?'結束回顧':'開始戰鬥 →'):'下一幕 →',null,'gold')+'</div></div></article>';
    }
