(function(ns){
  'use strict';
  const KEY='heroFrontierProfilesV1', VERSION='0.85.42';
  const clone=value=>JSON.parse(JSON.stringify(value));
  function profile(id,kind,round){return {id,kind,round,startedAt:new Date().toISOString(),startedVersion:VERSION,completed:[],encountered:[],discovered:[],cinematics:[],unlocks:{heroes:['hunter','naga'],factions:['hunter','naga'],maps:['beginner'],difficulties:['story','standard']},wallet:{gold:0,diamonds:0},data:{},lastRun:null};}
  class ProfileStore{
    constructor(storage){this.storage=storage;const raw=storage.getItem(KEY);this.serialized=raw;if(raw){this.state=JSON.parse(raw);this.validate(this.state);const next=clone(this.state);let changed=false;for(const p of Object.values(next.profiles).concat(next.archives)){if(!p.wallet){p.wallet={gold:0,diamonds:0};changed=true;}for(const [kind,id] of [['heroes','naga'],['factions','naga']])if(!p.unlocks[kind].includes(id)){p.unlocks[kind].push(id);changed=true;}}if(changed)this.persist(next);}else{const admin=profile('admin','admin',0),player=profile('test','test',1);for(const key of ['heroFrontierScoreRecordsV1','heroFrontierChapterCheckpointV1']){const value=storage.getItem(key);if(value!==null)admin.data['free:'+key]=value;}this.state={saveVersion:1,active:'test',nextRound:2,profiles:{admin,test:player},archives:[]};this.persist(this.state);}}
