(function(ns){
  'use strict';
  const KEY='heroFrontierProfilesV1', VERSION='0.85.1';
  const clone=value=>JSON.parse(JSON.stringify(value));
  function profile(id,kind,round){return {id,kind,round,startedAt:new Date().toISOString(),startedVersion:VERSION,completed:[],encountered:[],discovered:[],cinematics:[],unlocks:{heroes:['hunter'],factions:['hunter'],maps:['beginner'],difficulties:['story','standard']},wallet:{gold:0,diamonds:0},data:{},lastRun:null};}
  class ProfileStore{
    constructor(storage){this.storage=storage;const raw=storage.getItem(KEY);this.serialized=raw;if(raw){this.state=JSON.parse(raw);this.validate(this.state);const next=clone(this.state);let changed=false;for(const p of Object.values(next.profiles).concat(next.archives)){if(!p.wallet){p.wallet={gold:0,diamonds:0};changed=true;}}if(changed)this.persist(next);}else{const admin=profile('admin','admin',0);for(const key of ['heroFrontierScoreRecordsV1','heroFrontierChapterCheckpointV1']){const value=storage.getItem(key);if(value!==null)admin.data['free:'+key]=value;}this.state={saveVersion:1,active:'admin',nextRound:1,profiles:{admin},archives:[]};this.persist(this.state);}}
