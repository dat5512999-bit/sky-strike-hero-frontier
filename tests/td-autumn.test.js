...
test('暮秋遺跡為第三張獨立單路地圖，來回 U 彎不斷路且正確到城門',()=>{
 const ns=load(),map=ns.maps.definitions.autumn,png=fs.readFileSync(path.join(__dirname,'../'+map.asset));assert.equal(ns.maps.publicMaps().length,3);assert.equal(ns.config.routes.length,1);assert.equal(ns.config.sharedLength,0);assert.equal(png.readUInt32BE(16),1536);assert.equal(png.readUInt32BE(20),1024);
