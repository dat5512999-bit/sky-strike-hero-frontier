...
test('近戰士兵不追遠方敵人，只在部署射程內自動攻擊',()=>{const ns=load(),unit=new ns.entities.CombatUnit('shield',100,100),target=enemy(ns,220),shots=[];unit.cooldown=0;unit.update(.5,[target],shots);assert.equal(unit.x,100);assert.equal(unit.order,'idle');assert.equal(shots.length,0);target.x=130;unit.update(.1,[target],shots);assert.equal(unit.x,100);assert.equal(shots.length,1);});

test('失去目標後維持定點，索敵不觸發尋路',()=>{const ns=load(),unit=new ns.entities.CombatUnit('shield',100,100),target=enemy(ns,130);let attempts=0;const context={navigation:{findPath(){attempts++;return[];}},buildings:[]};unit.update(.1,[target],[],context);assert.equal(unit.lockedTarget,target);target.active=false;for(let i=0;i<12;i++)unit.update(.016,[target],[],context);assert.equal(unit.x,100);assert.equal(unit.lockedTarget,null);assert.equal(attempts,0);});

test('士兵部署座標永久固定，移動與攻擊移動命令皆無法改位',()=>{const ns=load(),navigation=new ns.systems.NavigationSystem(),commands=new ns.systems.CommandSystem(),unit=new ns.entities.CombatUnit('hunter',100,100),origin={x:unit.x,y:unit.y,guardX:unit.guardX,guardY:unit.guardY};assert.equal(commands.issue(unit,210,100,navigation,[]),false);assert.equal(unit.issueCommand('move',{x:310,y:100},[{x:310,y:100}]),false);assert.equal(unit.setTarget(400,100),false);simulate(unit,[],2,1,{navigation,buildings:[]});assert.deepEqual({x:unit.x,y:unit.y,guardX:unit.guardX,guardY:unit.guardY},origin);const near=enemy(ns,150),shots=[];unit.cooldown=0;unit.update(.016,[near],shots,{navigation,buildings:[]});assert.equal(shots.length,1);assert.equal(unit.orderLabel(),'永久定點');});

test('×2／×3 時間步進下士兵仍定點作戰',()=>{const ns=load();for(const speed of [2,3]){const unit=new ns.entities.CombatUnit('shield',100,100),target=enemy(ns,128),context={navigation:new ns.systems.NavigationSystem(),buildings:[]};unit.cooldown=0;const shots=simulate(unit,[target],.6,speed,context);assert.equal(unit.x,100);assert.ok(shots.length>0);target.active=false;simulate(unit,[target],2,speed,context);assert.equal(unit.x,100);assert.equal(unit.lockedTarget,null);}});
