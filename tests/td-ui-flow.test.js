...
function prototype(){
  ...
  for(const file of ['src/td/namespace.js','src/td/config.js','src/td/systems/TowerSkillSystem.js','src/td/systems/ResultScreen.js','src/td/TDGame.js'])vm.runInContext(fs.readFileSync(path.join(root,file),'utf8'),context,{filename:file});
...
test('開局、建造 Drawer 與統一選單保留同一套 UI 入口',()=>{
  ...
  assert.equal((html.match(/返回遠征選擇/g)||[]).length,2);
...
test('邊境對戰已從交付目錄與離線快取移除',()=>{
  for(const file of ['versus.html','versus.css','src/versus/namespace.js','src/versus/config.js','src/versus/entities.js','src/versus/AIStrategy.js','src/versus/VersusMatch.js','src/versus/main.js','assets/td/versus-battlefield-concept-v1.png'])assert.equal(fs.existsSync(path.join(root,file)),false,file);
  assert.doesNotMatch(fs.readFileSync(path.join(root,'sw.js'),'utf8'),/\.\/versus\.|\.\/src\/versus\/|versus-battlefield-concept-v1/);
});
...
test('勝敗結算使用共用結果面板並保留再挑戰入口',()=>{
  const game={report:{current:null,finalize:victory=>({score:victory?888:321,grade:victory?'A':'C',waves:victory?30:7,time:70,isNewBest:victory,persisted:true}),finalLine:()=> '本局 8 波｜最佳紀錄 888'},waves:{wave:8,catalog:{total:()=>30}},ui:{overlay:{hidden:true,dataset:{}},overlayTitle:{textContent:''},overlayMessage:{textContent:''},resultScore:{textContent:''},resultGrade:{textContent:''},resultHighlight:{textContent:'',hidden:true},finalReport:{textContent:''}}};
  ...
  assert.equal(game.ui.resultScore.textContent,'321');assert.equal(game.ui.resultGrade.textContent,'C');
  prototype().end.call(game,true);
  assert.equal(game.status,'victory');
  assert.match(game.ui.overlayMessage.textContent,/30 波/);
  assert.equal(game.ui.resultScore.textContent,'888');assert.equal(game.ui.resultGrade.textContent,'A');
  assert.match(game.ui.resultHighlight.textContent,/刷新本地圖最佳紀錄/);
