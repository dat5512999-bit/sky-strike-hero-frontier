...
test('本局積分使用右側小圖塊、可關閉面板並併入含清場時間的逐波戰報',()=>{const html=fs.readFileSync(path.join(root,'td.html'),'utf8'),main=fs.readFileSync(path.join(root,'src/td/main.js'),'utf8'),game=fs.readFileSync(path.join(root,'src/td/TDGame.js'),'utf8'),result=fs.readFileSync(path.join(root,'src/td/systems/ResultScreen.js'),'utf8');for(const id of ['td-score-shortcut','td-score-value','td-score-panel','td-score-close','td-score-total','td-score-grade','td-score-detail','td-score-best'])assert.equal(html.split('id="'+id+'"').length-1,1);assert.match(html,/🏆<\/b><small>積分/);assert.match(html,/<span>積分<\/span><span>清場時間<\/span><span>擊破<\/span>/);assert.match(main,/scoreShortcut\.addEventListener\('click'/);assert.match(game,/scoreState\(\)/);assert.match(game,/record\.timeBonus/);assert.match(result,/resultScore/);});
