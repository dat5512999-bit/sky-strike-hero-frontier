...
test('所有塔在建造卡上有不同的用途說明',()=>{const ns=load(),roles=Object.values(ns.config.buildings).map(tower=>tower.role);assert.equal(roles.length,18);assert.ok(roles.every(Boolean));assert.equal(new Set(roles).size,roles.length);});
