...
test('英雄塔防公開顯示目前程式與平衡資料版本',()=>{const html=fs.readFileSync(path.join(root,'td.html'),'utf8');assert.ok(html.includes('v'+JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8')).version));assert.match(html,/戰場可靠性修正/);assert.doesNotMatch(html,/SHARED UX/);});
