...
覆蓋新版檔案後重新載入，確認開局顯示 `v0.78.0 · BALANCE E2 · VERSUS MVP`，並可開啟 `versus.html`。英雄遠征戰績與章節檢查點無需遷移。
# v0.78.1 更新

更新後確認開局顯示 `v0.78.1 · BALANCE E2 · VERSUS ART`，並清除舊 Service Worker 快取或重新載入一次，以取得新版對戰背景與既有素材整合。
# v0.79.0 更新

重新載入後確認開局顯示 `v0.79.0 · BALANCE E2 · SHARED UX`。對戰頁應顯示新版頂部 HUD、右側快捷欄與底部建造抽屜。

# v0.79.1 更新

覆蓋新版 `td.html` 並重新載入；開局畫面應顯示 `v0.79.1 · BALANCE E2`，且不再顯示對戰原型入口。
