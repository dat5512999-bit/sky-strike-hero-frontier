...
新圖集是單張姿態配程序化動態，不宣稱完整逐幀動畫。商店／建造預覽與戰場共用同一圖集；正式 Map Asset 沒有改動。舊 faction-structures-v1 的三列按實際比例校準，修正戰鼓堡 Lv3 跨列露圖。

雷電、月弧、爆炸、魂、符文、冰晶與召喚成形由 Canvas 既有渲染層繪製；擴充方法、粒子預算與證據見 [Build V1](FACTION_BUILDS_V1.md)／[QA](RELEASE_V064_QA.md)。

## v0.44.1 巡林者換弓圖層

本次沿用 `hero-hunter-actions-unarmed-v4.png`、`class-weapons-atlas-v1.png` 與 `equipment-atlas-v1.png`，沒有新增 PNG。職業弓圖集每格為 512×約 341 的斜放商品圖，繪到英雄身上時需保留比例並按動作錨定右手；軍械庫獅心弓使用裝備圖集第一格替換職業弓。新增弓素材前需同時檢查三階武器、四格動作與左右鏡像。

## v0.40.1 真透明王國英雄圖集

`hero-hunter-actions-unarmed-v4.png`：由 v3 的 16 格守誓者・雷恩無武器圖，僅抽除烘入的灰白棋盤格；保留角色、姿勢與分層武器定位。尺寸 1252×1252、RGBA，每格 313×313；各格外側背景為 Alpha 0。v3 不再供遊戲載入。產圖提示：背景抽除、只移除棋盤、保持 4×4 角色與動作、不要新增武器、輸出真透明 PNG。
...
- `hero-rogue-actions-v2.png`：1254×1254 RGBA，紫黑夜刃指揮官 4×4 動作圖。
- `hero-rogue-actions-unarmed-v2.png`：同角色無武器底圖，供職業雙刃逐幀合成。
- `faction-towers-v2.png`：1254×1254 RGBA 九宮格；三欄為獅翼弩砲、月泉稜鏡、瘟疫尖碑，三列為初階、進階、終階。

所有圖片由內建 imagegen 以既有專案素材作鏡頭、比例與完成度參考，角色、建築、名稱與造型均為原創；提示要求固定格表、斜俯視、透明背景、無文字／Logo／浮水印及不複製既有遊戲角色。正式輸出已保存於 `assets/td/`，執行期不連線。
