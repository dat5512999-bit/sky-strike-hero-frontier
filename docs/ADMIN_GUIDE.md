...
> **v0.85.57 · 首領外觀：** 新增首領圖集按波次載入；檢查比較頁及素材失敗清單。不得把本次外觀验收視為整場 50 波發行驗收。 [交付與驗收](BOSS_VISUALS_V08557.md)。
...
> **塔樓特效 VFX 1.0.0（未發佈）：** 排查塔樓特效時確認 TowerVFX.js、入口與快取清單同版；先以 tower-vfx-preview.html 確認渲染，再檢查實際供電／支援來源。 見 [五族塔樓特效交付](TOWER_VFX_V1.md)。

> **v0.85.54：** 動畫略過提示只代表隔離了錯誤；請保留版本、技能、地圖、波次與 `towerFrontierGame.hero.nagaVfxFault` 堆疊。正常驗收不得產生此紀錄。 見 [娜迦技能 8D 分析與驗收](NAGA_SKILL_CRASH_8D_V08554.md)。

> **v0.85.55：** 後段威脅需逐一驗證：魔石不可全圖揭露、近戰不可對空、相位僅擋穿刺、鏡甲不傷英雄且單名士兵有反震冷卻。見 [管理驗收](ENEMY_THREATS_V08555.md)。

> **v0.85.53：** 發布前執行完整 50 波壓力測試與長局探針；不得只驗到第 30 波。若調整後段群組，維持第 30→31 波跌幅不超過 15%、標準第 40／50 波有效耐久至少 12.5／19 萬，並檢查濁潮小魔王的四族援軍。見 [管理驗收](FIFTY_WAVE_BALANCE_V08553.md)。

> **v0.85.53：** 管理者可開啟 `mobile-simulator.html` 檢查 iPhone 15 Pro Max 橫向手機 UI；此工具不寫入版型偏好，不可取代真機 Safari 與效能驗收。見 [手機預覽交付](MOBILE_SIMULATOR_V08551.md)。

> **v0.85.50：** 管理者以 `emberroad-grid-editor.html` 審核 2-2 格線；輸出的候選格不可跳過碰撞驗證直接寫入地圖。導入前需驗證道路距離、完整塔佔地、障礙、塔間距與手機鏡頭。見 [校對管理流程](EMBERROAD_GRID_EDITOR_V08550.md)。

> **v0.85.49：** 驗收旋潮領域時，以密集敵軍施放 W，確認每脈衝最多處理 12 名、沒有凍結遊戲迴圈。刻意製造敵軍傷害回呼或 Canvas 繪製錯誤時，僅領域中止並顯示安全提示；不得以未捕捉例外終止主迴圈。詳見 [旋潮防護說明](NAGA_MAELSTROM_FAILSAFE_V08549.md)。

> **v0.85.42：** 燼道相逢是 2-1 後的故事任務，管理者可直接驗收但一般玩家必須先完成前置。煙障是地圖暫態，不能用管理者存檔或自由遠征解鎖判定；依 [2-2 驗收](CHAPTER2_MAP_2_2_V08542.md) 檢查 +2／受控 +1。

> **v0.85.39：** 荒風隘道是西境地圖資料，不是管理者加成或玩家檔設定；管理者可直接驗收，但不得用其解鎖狀態判定一般玩家入口。見 [管理驗收](MAP_PRESSURE_V08539.md)。

> **v0.85.38：** 發布前執行 10／15 FPS 固定時序與四種瀏覽器採購探針；真機低幀率和真人平衡仍需另行驗收。見 [管理驗收](LONG_SESSION_FOLLOWUP_V08538.md)。

> **v0.85.36：** 發布需驗收六位英雄的進化變身與 Q／W／E／F 動畫，並確認其只讀視覺狀態、商城造型相容與同版 Service Worker。見 [動畫驗收](HERO_EVOLUTION_ANIMATION_V08536.md)。

> **v0.85.33：** 新安裝預設一般玩家，管理者檔仍可從設定切換；舊存檔不強制換檔。請驗收新玩家入口、任務獎勵提示、×3 持續低 FPS 重放及真機長局清單。見 [管理與驗收](PLAYER_ENTRY_FRAME_PACING_V08533.md)。

> **v0.85.31：** 管理者可開啟戰場效能面板，比較不同速度、怪物量與冰霜特效量；面板僅供診斷且不寫入存檔。見 [驗收流程](FRAME_PACING_V08531.md)。

> **v0.85.30：** 發布圖鑑軍團肖像時，六張 `assets/td/codex/faction-*-v2.jpg`、`FactionSystem.js`、`CodexCatalog.js`、`thumbnails.js` 與 `sw.js` 必須一起部署。以「世界圖鑑 → 軍團」確認六張均非空白、卡片與詳情相同；無新權限或資料遷移。見 [圖鑑軍團肖像交付](CODEX_FACTION_ART_V08530.md)。
> **v0.85.32：** 進化勳章與功勳分離；章節存檔需保留 `economy.emblems`、`hero.evolution`。Wave 50 超凡尚未對 30 波自由遠征開放。詳見 [英雄進化交付](HERO_EVOLUTION_V08532.md)。

> **v0.85.26：** 驗收機座 95G／1 木、容量 3 與供電 +22% 維持不變；無權限或遷移。見 [地精前期平衡](GOBLIN_EARLY_GAME_V08526.md)。

> **v0.85.25：** 管理者可用 `tests/td-directional-preview.html` 檢視上下動畫與腳底；部署需包含兩張新 PNG。詳見 [怪物方向動作與緩速步伐](COMBAT_MOTION_V08525.md)。

> **v0.85.24：** 管理者驗收地精常駐供能效果標記、上層連線與既有 Q／技師目標；無新增權限或遷移。見 [供能可讀性熱修](GOBLIN_NETWORK_VISIBILITY_V08524.md)。

> v0.85.20 怪物攻擊站姿與路徑銜接：參見 [完整操作、架構、部署、回復與驗收](COMBAT_MOTION_V08520.md)。

> **v0.85.19（2026-09-24）：** 部署時須連同教學腳本、軍械 UI、樣式與 `sky-strike-v0.85.19` 快取發布；不新增權限、後端或資料遷移。詳見 [新手裝備教學](CHAPTER1_ONBOARDING_V08519.md)。

> **v0.85.18（2026-09-24）：** 怪物視覺時序與守軍出手姿勢更新；戰鬥數值、座標和存檔不變。發版一併更新 `sky-strike-v0.85.18`。詳見 [戰鬥動作時序](COMBAT_MOTION_V08518.md)。

> **v0.85.17（2026-09-24）：** 發版時同步更新教學操作事件與 `sky-strike-v0.85.17`；首關補給僅限當局、無新權限或資料遷移。詳見 [教學操作修正](CHAPTER1_ONBOARDING_V08517.md)。

> **v0.85.15（2026-09-24）：** 發版時須同步更新教學腳本、故事波次與 `sky-strike-v0.85.15`；無新權限或資料遷移。詳見 [引導修正](CHAPTER1_ONBOARDING_V08515.md)。

> **v0.85.14（2026-09-24）：** 需整包部署第一章教學腳本、樣式與新版 Service Worker；無新權限或資料遷移。詳見 [第一章引導](CHAPTER1_ONBOARDING_V08514.md)。

> **v0.85.13（2026-09-24）：** 部署後抽查七種日蝕士兵與地精五兵的透明邊界、攻擊姿勢及奇克尺寸；無新權限。詳見 [軍團動作圖交付](FACTION_MOTION_V08513.md)。

> **v0.85.12（2026-09-24）：** 劇情卡沿用現有圖像與玩家檔案，不需新增設定；驗收首次序章後、已看過序章、跳過與回顧流程。詳見 [劇情卡交付](STORY_CARDS_V08512.md)。

> **v0.85.11（2026-09-23）：** 本次不需管理者設定或資料遷移；更新後確認排行榜與搖桿操作。 詳見 [手機版修復說明](MOBILE_RANKING_INPUT_V08511.md)。

> **v0.85.10（2026-09-23）：** 更新時略過舊 HTTP 快取並確認啟用版號；排查舊畫面時使用 update.html，無需清除玩家資料。 詳見 [更新修復說明](UPDATE_RECOVERY_V08510.md)。

> **v0.85.9：** 管理者驗收選卡後地圖保持清晰、指標移入才顯示局部預覽，並抽查道路／地形／占位與扣款規則。無新增權限。詳見 [地圖部署視覺整理](DEPLOYMENT_VISUAL_V0859.md)。
...
> **v0.85.6：** 速度基準在 `TDGame.loop` 乘 2，高速時每步最多 33 毫秒；發布時驗證三段速度、暫停與低階裝置效能。見 [速度調整](GAME_SPEED_V0856.md)。
...
v0.85.3：三張劇情地圖須連同新版 PNG、地圖資料、建塔判定、地面提示與 Service Worker 一起更新；重開舊分頁後逐圖測試。見 [戰場維護與回復](STORY_MAP_REFINEMENT_V0853.md)。

v0.85.2：邊境對戰原型已移除；部署檢查舊檔與快取，開發圖鑑只供本機。見 [移除與驗收紀錄](VERSUS_REMOVAL_V0852.md)。

v0.84.9 的軍械庫為本局裝備清單；販售金幣進本局資源，裝備與存檔格式沿用既有設計。管理與回復見 [軍械庫更新](ARMORY_V0849.md)。

v0.84.8 地圖資產、任務解鎖與回歸驗證見 [三張劇情戰場](STORY_MAPS_V0848.md)。

> **v0.84.7 現行商城：** 四位英雄外觀（獸人為「冥骨帝王」）與「日蝕王庭」已開放使用玩家檔案的試用水晶購買、保存與裝備；沒有真實付款。詳細規格、操作、限制、部署與測試見 [外觀上架交付](COSMETIC_RELEASE_V0847.md)。

> v0.84.6：使用新玩家測試輪次驗證「烽火初燃」首通解鎖；管理者已全解鎖帳號只顯示「已解鎖戰場」。先確認存檔成功，再檢查主要按鈕會帶到雙隘口要塞的出征選擇。詳見 [結算交付](RESULT_SCREEN_V0846.md)。

> v0.84.4：排行榜讀 `ProfileStore.adapter('free')` 中的 `heroFrontierScoreRecordsV1`；勝、敗、撤退均保留，玩家檔案隔離。`profileArt` 為可選玩家欄位；未推出造型不開放購買。詳見 [本版維護](EXPEDITION_SCORE_PROFILE_V0844.md)。

> v0.84.3：大酋長 E 改由 ChiefShockwave 管理；部署需包含新 JS、SVG 與 SW，無帳號或資料庫設定。詳見 [技能交付](CHIEF_SHOCKWAVE_V0843.md)。

> v0.84.2：排行榜僅讀目前玩家檔案的 free SCORE_V1 紀錄，測試輪次互相隔離；預覽不寫存檔。資料異常時先匯出備份。詳見 [排行榜管理](RANKING_UI_V0842.md)。

> v0.83.6：使用者的舊 mobile 設定在寬滑鼠桌機改以 auto 判斷；新版手動選擇仍保留。不要以清除玩家存檔處理版型問題。 詳見 [右鍵與戰鬥版型修正](BATTLE_INPUT_LAYOUT_V0836.md)。

> v0.83.5：圖鑑接目前 ProfileStore；管理者與測試輪次沿用既有權限。分支獨立圖鑑資料不自動匯入。展示商城每次進入重設，不提供真實付款或正式外觀持有資料。 詳見 [商城與圖鑑整合交付](FEATURE_INTEGRATION_V0835.md)。

> v0.83.4：大廳版型改依輸入能力與寬度判斷；無帳戶／權限／餘額遷移。手機維持原有橫向遊戲殼。驗收與設定見 [大廳調整](LOBBY_LAYOUT_V0834.md)。

> v0.83.3：序章紀錄按管理者／測試輪次隔離；新輪次會重新播放，舊輪次保留收藏。素材放 assets/cinematics/prologue；不需資料庫或帳號權限。角色／地點 Reference 為 pending-approval，未核准不可當正式美術使用。見 [管理與素材交付](PROLOGUE_CINEMATIC_V1.md)。

> v0.83.2：三個壓力波次出怪間隔為原值 ×1.2／×1.2／×1.3。固定防線無英雄技能的自動測試仍會失守，故本版是節奏試調，不是平衡完成。後續請比較不同合法配隊的漏怪、清場時間與金幣，勿單憑單一測試再降全局血量。

> v0.83.1：固定單體／群怪／重甲場景分別比較擴軍與升級，不只看面板 DPS。塔首級 ×1.6、火槍手首級 ×0.8、獵手箭數 2／2／3／4／4；金幣功勳兌換與資料格式不變。
...
v0.80.0 的 `wild` 原生名冊由 `FactionSystem` 管理。`orc-contract` 必須保留 `legacy:true` 供舊紀錄查名，但不得重新加入 `LootSystem.createOffers()` 或第 5 波 milestone。荒野平衡調整應同時檢查戰鼓 8/3 秒週期、薩滿 12% 增傷及戰牛 290G／4木門檻。


## 0.83.0 · 大廳與獨立測試輪次

管理者可使用所有英雄、軍團、地圖和難度軍備，正常消耗資源。身份僅限本機，不可作為未來線上授權依據。測試存檔獨立。 完整操作、限制、架構與回復步驟見 [P0 說明](LOBBY_P0_V083.md)。

## v0.83.3

管理者與每輪測試各有 wallet；新 ROUND 歸零但封存保留原餘額。尚未提供發放或消費管理操作。詳見 [v0.83.3](LOBBY_EXPEDITION_V0833.md)。

# 地精工程團測試入口

切換管理者檔，於自由遠征測試任意英雄 × 地精軍團，或地精英雄 × 任意軍團。正式玩家檔由既有 ProfileStore unlocks 鎖定；不要以管理者可遊玩視為已通過劇情解鎖。正式開放須由核准的故事里程碑分別寫入 `heroes:goblin` 與 `factions:goblin`，本版沒有指定 Chapter。詳見 [地精 V1](GOBLIN_FACTION_V1.md)。

驗收荒野及地精英雄時，請依 [v0.84.5 武器紀錄](FACTION_HERO_WEAPONS_V0845.md) 檢查三階商店、戰場武器、存檔與軍團自由搭配。
...
> v0.85.4 命中回饋不需設定或資料遷移；管理者可用不同攻擊類型確認方向火花與視覺後仰。維護與驗收見 [命中回饋交付](HIT_FEEDBACK_V0854.md)。
> v0.85.1：根目錄 `td.html` 已整合霜牙肖像與霜原盟族群像；素材、部署、回復與驗收見[選角插畫交付](FROSTLAND_SELECTION_ART_V0851.md)。
